...
export default function AdminDashboard(root) {
  ...
  function setButtonLoading(btn, loading, text = 'Processing...') {
    ...
    if (loading) {
      ...
      btn.disabled = true;
      btn.innerHTML = `<ion-icon name="hourglass-outline" class="mr-1 animate-pulse"></ion-icon>${text}`;
    } else {
          ...
          <div class="space-y-6">
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                ...
                <button class="bg-brand px-4 py-2 rounded text-white" id="refreshBooths">Refresh</button>
              </div>
            </div>
          ...
          <div class="space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center gap-2">
                  <label class="text-glass-secondary text-sm">Search:</label>
                  <input id="paymentSearch" type="search" placeholder="Name or email..." class="bg-glass-surface border border-glass-border rounded px-3 py-2 text-glass text-sm w-48" />
                </div>
                <div class="flex items-center gap-2">
                ...
                <button class="bg-brand px-4 py-2 rounded text-white" id="refreshPayments">Refresh</button>
            ...
            <div id="paymentStats"><!-- Stats will load here --></div>
  ...
  async function initializeTab() {
    switch (activeTab) {
      ...
      case 'vendors':
        ...
        const refreshVendorsBtn = root.querySelector('#refreshVendors');
        if (refreshVendorsBtn) refreshVendorsBtn.onclick = () => loadVendors();
        break;
      case 'users':
        await loadUsers();
        // Refresh button handler
        const refreshUsersBtn = root.querySelector('#refreshUsers');
      ...
      case 'booths':
        ...
        setupBoothButtons();
        // Refresh button handler
        const refreshBoothsBtn = root.querySelector('#refreshBooths');
        if (refreshBoothsBtn) refreshBoothsBtn.onclick = () => loadBooths();
        break;
      case 'payments':
        await loadPayments();
        // Refresh button handler
        const refreshPaymentsBtn = root.querySelector('#refreshPayments');
        if (refreshPaymentsBtn) refreshPaymentsBtn.onclick = () => loadPayments();
        break;
      case 'admins':
        await loadAdmins();
        break;
  ...
  if (exportBtn) exportBtn.onclick = () => exportCsv(`vendors_${new Date().toISOString().slice(0,10)}.csv`, lastVendors);
      ...
      // Checkbox change listener - only add once
      ...
      // Search input listener - only add once
      ...
      vendorsList.addEventListener('click', async (e) => {
        ...
        try {
          ...
          if (action === 'view' || action === 'edit') {
            setButtonLoading(el, false); // Reset immediately for view/edit
            ...
            setButtonLoading(el, false); // Reset for pay modal
            await showStripePaymentModal(vendorId, vendorName, vendorEmail);
          } else if (action === 'delete') {
            setButtonLoading(el, false); // Reset before dialog
  ...
  if (exportBtn) exportBtn.onclick = () => exportCsv(`users_${new Date().toISOString().slice(0,10)}.csv`, lastUsers);
      ...
      // Hook up search/filter controls - only add once
  ...
  async function loadPayments(filterType = 'all', searchTerm = '') {
    ...
    try {
      ...
      const getStatusBadge = (status) => {
        if (status === 'paid') return '<span class="px-2 py-1 bg-green-600 rounded text-white text-xs">Paid</span>';
        if (status === 'payment_sent') return '<span class="px-2 py-1 bg-yellow-600 rounded text-white text-xs">Invoice Sent</span>';
            ...
            <div class="glass-card p-4 ${payment.paymentStatus === 'paid' ? 'border-l-4 border-green-500' : payment.paymentStatus === 'payment_sent' ? 'border-l-4 border-yellow-500' : 'border-l-4 border-red-500'}">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-glass-secondary text-sm">${payment.contactEmail}</p>
                ...
                <div class="text-right">
                  ...
                  <div class="flex gap-2 mt-2">
                    ${payment.stripeInvoiceUrl ? `<a href="${payment.stripeInvoiceUrl}" target="_blank" rel="noopener" class="px-3 py-1 bg-orange-600 rounded text-white text-sm">View Invoice</a>` : ''}
                    ${payment.paymentStatus !== 'paid' ? `<button class="px-3 py-1 bg-brand rounded text-white text-sm" data-action="send-payment" data-vendor-id="${payment.id}" data-vendor-name="${payment.name}" data-vendor-email="${payment.contactEmail}">Send Invoice</button>` : ''}
                  </div>
                </div>
              </div>
            </div>
      ...
      // Search/filter listeners - only add once
  ...
  function showVendorProfileModal(vendorId, vendor, isEditMode) {
    ...
    // Close modal function (scoped to this modal instance)
    const closeModal = () => {
    ...
    // Form submit handler for edit mode
    if (isEditMode) {
